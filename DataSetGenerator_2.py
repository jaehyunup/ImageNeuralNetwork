...
for file_image in filename_in_dir:
    ...
    for batch in rotateGenerator.flow(x, save_to_dir='img_data\\img_data_rotate\\frame0000%d' % a,save_prefix='frame0000%d' % a, save_format='jpg'):
        ...
        if i1>9 :
            i1=0
            break
    for batch in shearGenerator.flow(x, save_to_dir='img_data\\img_data_shear\\frame0000%d' % a,save_prefix='frame0000%d' % a, save_format='jpg'):
        i2 += 1
        if i2 > 9:
            i2 = 0
            break
    for batch in shiftGenerator.flow(x, save_to_dir='img_data\\img_data_shift\\frame0000%d' % a,save_prefix='frame0000%d' % a, save_format='jpg'):
        i3 += 1
        if i3 > 9:
            i3 = 0
            break
